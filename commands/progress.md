...
2. **Active lesson** (if `state.json` has a topic) — topic, difficulty, and progress
   (`completedSteps` count of `steps` total), the current step description, current
   `currentPhase` (Mini Lesson or Problem Solving), and how many hints were used on it. If no
   lesson is active, say so and suggest `/learn <topic>`.
